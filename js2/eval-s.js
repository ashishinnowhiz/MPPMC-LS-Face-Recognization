...
function currentPage(){
    ...
    var pageHeight = 19 + parseInt(document.getElementById(selectedTab+"PageDiv1").offsetHeight);
    var scrollPosition=pageHeight+scrollDiv.scrollTop;
    var inPage = Math.floor(scrollPosition / pageHeight);
...
function addUndo(lastPage,lastAction){
    redoStack= new Array();
    var undoMax=6;
    if(undoStack.length>5){
        //undoStack.shift(); //removes first element
    }
    undoStack.push(lastPage+"/"+lastAction);
}
...
function undo(){
    if(undoStack.length>0){
        ...
        toUndo=undoAction.split("/");

        var undoAction=getContent('sheet/undo?action='+toUndo[1]+'&page='+toUndo[0]);
        var data=getContent('sheet/pageLog/'+toUndo[0]);
        var action=undoAction.action;
        ...
        clearCanvas('page'+toUndo[0]);
        drawActions(data.actions,false);
    }else{
...
function redo(){
    if(redoStack.length>0){
        ...
        toRedo=redoAction.split("/");
        var data=getContent('sheet/redo?action='+toRedo[1]+'&page='+toRedo[0]);
