...
{
     public function get_pending_allocations(){
        $this->db->select('allocation_id,allocation_file');
        $this->db->from('allocation_log');
     ...
     public function getallocationsheet($allocation_id){
        $this->db->select('sheet_file');
        $this->db->from('sheets');
	...
	public function get_datewise_evalution($where_array = '')
    {
                    ...
                    $this->db->from('sheets');
                    $this->db->join('evaluation', 'evaluation.sheet_file = sheets.sheet_file');
                    $this->db->join('subjects', 'subjects.subject_code = sheets.subject_code');
                    $this->db->join('mediums', 'mediums.medium_code = sheets.medium_code');
                    $this->db->group_by(array('evaluation.evaluation_date',"subject_code","medium_code"));
                    $this->db->order_by('evaluation.evaluation_date', "subject_code", "medium_code");
                    $query = $this->db->get();
    ...
    //code update by ashish 05-05-2026
...
//     }
//     $this->db->trans_start(); 
//     $this->db->select('sheet_file,eval_time,user_name');
//     $this->db->where('user_role', 'Evaluator');
//     $evalQuery = $this->db->get('eval')->result_array();
//     $done = [];
//     if(!empty($evalQuery)){
//     foreach ($evalQuery as $row) {
//         $done[] = $row['sheet_file'];
//         if (!empty($row['eval_time'])) {
//             $eval_date = date('Y-m-d', strtotime($row['eval_time']));
//             $file_path = FCPATH . "answersheet/upload/{$eval_date}/" . 
//                          $row['user_name'] . "_" . 
//                          $row['sheet_file'] . "_checked.pdf";
...
//     }
//     if (!empty($done)) {
//         $this->db->where_in('sheet_file', $done);
//         $this->db->where('user_role', 'Evaluator');
//         $this->db->delete('eval');
//         unset($done);
//     }
...
//    $this->db->from('sheets'); 
//    $this->db->join('evaluation', 'sheets.sheet_file = evaluation.sheet_file','left'); 
//    $query = $this->db->get(); 
//    $result = $query->result_array(); 
//    $done=array(); 
//    foreach ($result as $row) { $done[]=$row['sheet_file']; }
//     $query = $this->db->query($this->db->get_compiled_select() . " FOR UPDATE");
//     $row = $query->row_array();
//     if (empty($row)) {
...
//     }
//     $updateData = [
//         'sheet_status'       => 'Assigned',
//         'examiner_username' => $evaluator['examiner_username'],
//         'evaluator_code'    => $evaluator['evaluator_username'],
//         'sheet_assign_time' => date('Y-m-d H:i:s')
//     ];
//     $this->db->where('sheet_id', $row['sheet_id']);
//     $this->db->where('sheet_status', 'Pending'); // 🔒 safety
//     $this->db->update('sheets', $updateData);
//     if ($this->db->affected_rows() !== 1) {
//         $this->db->trans_rollback();
//         return null;
//     }
//     $this->db->trans_complete(); 
//     // 6. Return assigned sheet
//     return $this->get_file($evaluator['evaluator_username']);
//   } 
  //code for rechec checked booklet
   public function get_evaluation_sheet($evaluator, $where_array)
    {
                    $sheet=$this->get_file($evaluator['evaluator_username']);
  ...
  //change done here  
    ...
    {
                    $this->db->select('sheet_id,sheet_file,allocation_id,sheets.medium_code,sheets.paper_code,sheets.subject_code,sheet_status,sheet_assign_time,examiner_username,paper_model_question, paper_model_answer,paper_total_marks, sheet_eval, sheet_json_marks,sheet_save_time');
    ...
    {
                        $this->db->select('sheet_id,sheet_file,allocation_id,medium_code,paper_code,subject_code,sheet_status,evaluation_date,recheck_assign_time AS sheet_assign_time,evaluator_code');
    ...
    {
                        $this->db->select('sheet_id,sheet_file,allocation_id,medium_code,paper_code,subject_code,sheet_status,evaluation_date,remark_assign_time AS sheet_assign_time');
                        $this->db->where('marker_username', $examiner);
                        $this->db->where('sheet_status', 'Rechecked');
                        $this->db->where('marker_evaluation_marks', 0);
    ...
    //reval code
     public function update_reevaluation_sheet2($user_id, $data,$filecount)
    {
        $this->load->model('EvaluationModel');
        $evaluations=$this->EvaluationModel->get_evals2($data['sheet_file']);
                    // echo "<pre>".$user_id;print_r($data);print_r(count($evaluations));die;
                    $marks=$data['evaluation_marks'];
                    unset($data['evaluation_marks']);
                    $sheet=$data;
                    //condition if last 2 marks  variation of more than 20%
                    // if(count($evaluations)>1){  //update this 1 to 2 for evaluation in 3 markers 
                    //     $sheet['sheet_status']='Checked';
                    // }else{
                    //     $sheet['sheet_status']='Pending';
                    // }
                    //third evaluation check condition 
                     if(count($evaluations)>1 && count($evaluations)==$filecount){  //update this 1 to 2 for evaluation in 3 markers 
                        $sheet['sheet_status']='Checked';
                    }else{
                        $sheet['sheet_status']='Pending';
                    }
                    ...
                    $sheet['evaluation_date']=date('Y-m-d');
                    $sheet['sheet_updated_by']=$user_id;
                    $key = $this->config->item('encryption_key');
                    $this->db->set('evaluation_marks', "AES_ENCRYPT('{$marks}',UNHEX(SHA2('".$key."',512)))", false);
                    $this->db->set('final_marks', "AES_ENCRYPT('{$marks}',UNHEX(SHA2('".$key."',512)))", false);
