...
$data['url']=base_url()."answersheets/uploads/".$_GET['date']."/".$path_parts['filename']."_checked.".$ext;
    ...
    public function page($page = 0)
    {
        ...
        if (isset($_GET['evaluator']) && ($_GET['evaluator']!='')) {
            $where['evaluator_code']=$_GET['evaluator'];
        }
            $data['date']=date('Y-m-d', time());
            $where['sheet_status']='Checked';
            $where['head_evaluation_marks']=0;
            /*
        ...
        if (isset($_GET['sheet']) && ($_GET['sheet']!='')) {
            ...
            $config['base_url'] = base_url().'recheck/page/';
            $config['total_rows'] = $this->SheetsModel->get_sheets_count($where, $like);
            $config['per_page'] = 100;
            ...
            $data['sheets'] = $this->SheetsModel->get_sheets($limit, $where, $like);
    ...
    public function updateSheet()
    {
        ...
        $data = ['recheck' => $val,'recheck_remarks' => $remarks,'examiner_username'=>$_SESSION[$this->config->item('exam')['exam_session']]['user_name']];
