  ...
  //		echo "MAC address of client is: $MAC";
        ...
        if($_SESSION[$this->config->item('exam')['exam_session']]['user_role']=='Coordinator'){
			...
			//echo "Welcome";
			 ...
			 if($obj->status=='D')
			 {
				$_SESSION['logouts']='Something went wrong. Please contact the admin123';
				 redirect('Logout');
			 }
			//echo $obj->status;
			//echo $obj['status'];
        ...
        $eval_array = array();
           ...
           $data['subject_code'] = "SubjectCode";
            ...
            foreach($evaluator as $eval){
               ...
               $data['subject_code'] = $eval['subject_code'];
...
$data['date'] = "Date";
            ...
            foreach($evaluator as $eval){
                ...
                $data['branch_code'] = "'".substr($eval["BranchCode"], 2);
                $data['course_name'] = $eval["CourseName"];
                $data['course_code'] = "'".$eval["CourseCode"];
    ...
    //Code End Here
     ...
     public function add_image(){
        ...
        $booknames = pathinfo($bookname, PATHINFO_FILENAME);
		...
		$targetDir = "uploads/profile_pic/".$user_role."_". $user_id."/".date('d-m-Y')."/"; 
				if(is_dir($targetDir))
				{} 
				else 
				...
				$filename = $user_role."_".$user_id."_".time() ."_".$booknames.'.jpg'; 
				...
				if (file_put_contents($filePath, $decodedImageData)) { // file_put_contents for upload image on directroy
                    ...
                    $activity['user_email'] =  $_SESSION[$this->config->item('exam')['exam_session']]['username'];
                    $activity['user_role'] = $_SESSION[$this->config->item('exam')['exam_session']]['user_role'];
                    $activity['bookname'] = $booknames;
                    $activity['image_file'] = $filePath;
                    $activity['responce'] = $responce;
                    $activity['status'] = $status;
                    $activity['add_time'] = date('y-m-d H:i:s');
                    $data=$this->EvaluatorsModel->add_book_activity($activity);
                  

                    $_SESSION[$this->config->item('exam')['exam_session']]['start_marking_auth']='success';
