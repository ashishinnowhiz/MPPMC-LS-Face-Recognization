							...
							<select name='Evaluator' class='form-control' onchange="SelectEvaluator(this.value)" id="Evaluator">
								...
								<?php
									...
									foreach($evaluators AS $evaluator){
										echo "<option value='".$evaluator['evaluator_username']."'";
										if($evaluator['evaluator_username']==$evalr){
											echo " selected='selected'";
										}
										echo ">".$evaluator['evaluator_username']."</option>";
									}
								?>
							</select>
										...
										#var_dump($sheets);
											...
											foreach($sheets AS $sheet){
												...
												echo "<tr>
													...
													echo "<td> <input data-validation='required length alphanumeric' data-validation-allowing=' ' name='recheck_".$sheet['sheet_id']."_remarks' maxlength='100' id='recheck_".$sheet['sheet_id']."_remarks' value='".$sheet['recheck_remarks']."'  ".$checked." type='text' placeholder=\"Enter Remarks\"  /> <input name='recheck_".$sheet['sheet_id']."' id='recheck_".$sheet['sheet_id']."' value='1' ".$checked." type='checkbox' onclick=\"return updateRecheck(this)\" />  </td>";
													} 
...
function updateRecheck(ele){
	...
	var status = $(ele).prop("checked");
		...
		$.ajax({
			type: 'GET',
			dataType: 'json',
			url: base_url+"recheck/updateSheet/",
			data: strData,		
...
function SelectDay(day){
	...
	window.location='<?php echo base_url(); ?>recheck?evaluator='+evaluator+'&day='+day;
}
function SelectEvaluator(evaluator){
	date= $("#basic-datepicker").val();
	
	window.location='<?php echo base_url(); ?>recheck?evaluator='+evaluator+'&date='+date;
}
</script>
...
function onSelectDate(){
	...
	window.location=baseurl+"recheck?evaluator="+evaluator+"&date="+dateVal;
