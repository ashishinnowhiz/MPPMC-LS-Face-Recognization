    ...
    if(display=='none'){
        ...
        var y=drag.top-offset.top;
            ...
            if(commentText.length>0){
				...
				/*var resp = getContent('sheet1/addlog', obj);
                if(resp.success){
...
function saveMarkingLog(){
	alert('Save Marking');
	var url=base_url+'sheet1/saveMarkingLog';
	$.ajax({
                type: "POST",
                url: url,
...
function saveImages(){
	var urlSheet=base_url+'sheet1/saveMarkingLog';
	var minTime=sheet.min_eval_time * 60;
    
        var unMarked=[];
        var url=base_url+'sheet1/saveImage';
...
function finalSave(){
    ...
    if($('#confirmation').prop("checked") == true){
        //var resp=getContent('sheet1/savePdf?url='+sheet.url); 
        //alert('sheet1/savePdf?url='+sheet.url);
        var resp='';
            $.ajax({
                type: "POST",
                url: base_url+'sheet1/savePdf?url='+sheet.url,
                async : false,
                data: { 
                    'json' : schemeObj,
                    'sheetuser' : sheet.evaluator_code,
                    'edate' : sheet.evaluation_date,
                    'url' : sheet.url 
                }
                }).done(function(data) {
                    resp=data;
            ...
            if(resp.success){
					...
					if(save.success){
						getContent('sheet1/deletePages?url='+sheet.url,'',true);
...
function rejectSheet(){
    ...
    if(error !=''){
        ...
        var resp=getContent('sheet1/reject?sheet_file='+sheet.sheet_file+'&reason='+reason+'&other='+otherText);
        alert(resp.message);
        if(resp.success){
            top.location.href='';
        }
...
function getSheet() { 
    ...
    if(sheet.success){
       ...
       // $('#verifyCheckedSheet').modal('show');
        ...
        var details=getContent('sheet1/details?url='+sheet.url);
        var res=getContent('sheet1/resolution?url='+sheet.url);
        resetCanvas(res.size.width, res.size.height);

        var scheme=getContent('sheet1/scheme?paper_code='+sheet.paper_code);
        schemeObj=scheme.json;
        groups = {};
        //$('#docPageDiv1 img').attr('src', 'sheet1/page?num=1&url='+sheet.url);
        ...
        for (i = 1; i <= totalPages['doc']; i++) {
            $('#docPageDiv'+i).show();
            //$('#docPageDiv'+i+' img').attr('src', 'sheet1/page/'+(i-1)+'?url='+sheet.url);
            pageContent += "<a id='docpageNum"+i+"' onclick=\"goToPage('"+i+"')\" class='pageNo'>"+i+"</a>";
            //var data=getContent('sheet1/pageLog/'+i);
            //drawActions(data.actions);
        }
        $('#docPageNos').html(pageContent);
        
        $('#docPageDiv1 img').attr('src', base_url+'sheet1/page/0?url='+sheet.url);
        //$('#docPageDiv1 img').attr('src', base_url+'sheet1/page/1?url='+sheet.url);
        //first page actions draw
        var data=getContent('sheet1/pageLog/1');
        drawActions(data.actions);
        ...
        */
        for (i = 2; i <= totalPages['doc']; i++) {
            //var data=getContent('sheet1/pageLog/'+i);
            //drawActions(data.actions);
          //  $('#docPageDiv'+i+' img').attr('src', base_url+'sheet1/page/'+(i)+'?url='+sheet.url);
            $('#docPageDiv'+i+' img').attr('src', base_url+'sheet1/page/'+(i-1)+'?url='+sheet.url);
            getContent('sheet1/pageLog/'+i,'',true,'loadActions');
        }

        
        $("#evalMessage").html('Loading model answer, Please wait ...');
        getContent('sheet1/details?url='+sheet.paper_model_answer,'',true,'loadModel');

        //$("#evalMessage").html('Loading paper, Please wait ...');
        //getContent('sheet1/details?url='+sheet.paper_model_question,'',true,'loadPaper');
...
function loadModel(ansDetails){
    ...
    for (i = 1; i <= totalPages['ans']; i++) {
        $('#ansPageDiv'+i).show();
        $('#ansPageDiv'+i+' img').attr('src', base_url+'sheet1/page/'+(i-1)+'?url='+sheet.paper_model_answer+'&&ans=ans');
...
function loadPaper(queDetails){
    totalPages['que']=queDetails.pageCount;
        ...
        for (i = 1; i <= totalPages['que']; i++) {
            $('#quePageDiv'+i).show();
            $('#quePageDiv'+i+' img').attr('src',base_url+'sheet1/page/'+(i-1)+'?url='+sheet.paper_model_question);
...
function rotate(){
    ...
    var data= getContent('sheet1/rotate/'+(i-1)+'?url='+sheet.url);
...
if(data.status){
    ...
    //$('#docPageDiv'+i+' img').attr('src', 'sheet1/page/'+(i-1)+'?refresh='+refresh+'&url='+sheet.url);
}
