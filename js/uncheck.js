...
function saveMarkingLog(){
	alert('Save Marking');
	var url=base_url+'sheet/saveMarkingLog';
	$.ajax({
                type: "POST",
                url: url,
                data: {sheetObj:sheetMarking}
                }).done(function(o) {
                //alert(o); 
				return true;
                });
...
function saveImages(){
	var urlSheet=base_url+'sheet/saveMarkingLog';
...
function rejectSheet() {
    ...
    } else if (reason == 'Other' || reason == 'Different Language' || reason == 'Different Set' || reason == 'Different Subject' || reason == 'Page Are Blur') {
...
function reasonChange(reason) {
    if (reason == 'Other' || reason == 'Different Language' || reason == 'Different Set' || reason == 'Different Subject') {
        $('#rejectOther').show();
    } else {
        $('#rejectOther').hide();
    }
    ...
    if(sheet.success){
       ...
       $('#subjectName').html(sheet.subject_name);
        ...
        //$('#docPageDiv1 img').attr('src', 'sheet/page?num=1&url='+sheet.url);
            $("#recheckComments").html(sheet.comment);
        var table = document.getElementById("markScheme");
        ...
        var data=getContent('sheet/pageLog/1');
		 for (k = 0; k < data.actions.length; k++) {
            sheetMarking.push(data.actions[k]);
        }
        drawActions(data.actions);
	...
	//console.log("drawAction"+JSON.stringify(action));
            ...
            if(drawTool=='score'){
				...
				if(action.user_role!='Head_Evaluator'){
                    ...
                    if(allotedMarks[action.eval_que_index]=='NA' || allotedMarks[action.eval_que_index]==''){
...
function remove(){
    ...
    if(sheetMarking.length>0){
		...
		console.log("undoActionData:"+JSON.stringify(undoActionData));
		
		
		if(undoActionData.length>0){
			let text = "Are sure remove the Question No: "+(schemeObj[selectedQue].Question_No)+" Marks?.";
			if (confirm(text) == true) {
			 for(var index=0;index<undoActionData.length;index++){
				...
				if(action.eval_action=='score'){
						var detail=JSON.parse(action.eval_details);
						var score = parseFloat(detail.score);
						allotedMarks[action.eval_que_index]= 'NA';
						
						$('#queMark'+action.eval_que_index).html('');
						setScore();
						calculateTotal();
				} 
				...
				var ppage=undoActionData[index].eval_page;
				 ...
				 var sheetM=sheetMarking.filter(function(v, i) {
					return v["eval_page"] == undoActionData[index].eval_page;
				}); 
				drawActions(sheetM,false);
...
function undo(){
    ...
    if(sheetMarking.length>0){
        ...
        if(action.eval_action=='score'){
                var detail=JSON.parse(action.eval_details);
                var score = parseFloat(detail.score);
				var m=allotedMarks[action.eval_que_index] - score;
				
					allotedMarks[action.eval_que_index]=m==0?'NA':m;
					$('#queMark'+action.eval_que_index).html(m==0?'':m);
