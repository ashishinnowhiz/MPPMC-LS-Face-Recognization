    ...
    if (dragStart) {
        ...
        if (commentText.length > 0) {
            ...
            var obj1 = {
                ...
                eval_time:new Date().toISOString().split('T')[0],
                eval_date_time:currentDateTime2(),
              // eval_date_time: DateFormat('yyyy-MM-dd').format(DateTime.now()),
            };
    ...
    if (minTime > timespent) {
        ...
        for(k=0;k<schemeObj.length;k++){
            if(schemeObj[k].Question_No=='condition'){
                ...
                //console.log('1',schemeObj[k].allotedMarks);
                if(schemeObj[k].allotedMarks !== undefined){
                    if((schemeObj[k].allotedMarks==='' || schemeObj[k].allotedMarks==='NA') && schemeObj[k].Group==''){
                     ...
                     // console.log('2',schemeObj[k].Group);
        ...
        if (resp.success) {
            ...
            if (!isEval) {
                ...
                if (save.success) {
					...
					getContent('sheet/deletePdf?url='+sheet.sheet_file,'',true);
...
function allotMark(score) {
    ...
    if (commentText != '') {
        ...
        drawCanvas('time', canvasId, x + 30, y + 22, width, height, new Date().toISOString().split('T')[0]);
        /* var resp = getContent('sheet/addlog', obj);
        if(resp.success){
            scorePage[selectedQue]=canvasId.substr(4);
...
function getSheet() {
    ...
    if (sheet.success) {
        ...
        var tableSum = document.getElementById("summaryTableHead");
