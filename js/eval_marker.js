    ...
    if (dragStart) {
        ...
        if (commentText.length > 0) {
            ...
            /* var resp = getContent('sheet/addlog', obj);
                if(resp.success){
                    addUndo(pageId.substr(4),resp.id);
                    drawCanvas(drawTool,pageId,x,y,width,height,commentText);
                }else{
                    alert(resp.message);
                } */
...
function drawCanvas(toDraw, pageId, x, y, width, height, commentText = '', userName = '') {
    ...
    if (userName != 'Evaluator' && userName != 'Head_Evaluator') {
        pageMarked[pageNum] = true;
        $('#docpageNum' + pageNum).addClass('headMarked');
    }
...
function saveImages() {
    ...
    if (minTime > timespent) {
        var msg = 'Evaluation time for this answersheet is set to ' + toHHMMSS(minTime) + '.';
        msg += '\nYour evalaution time is only ' + toHHMMSS(timespent) + '.';
        msg += '\nPlease save after set minimum evaluation time'
        alert(msg);
    } else {
			var unmarkquestion=[];
			for(k=0;k<schemeObj.length;k++){
				if(schemeObj[k].Question_No=='condition'){
					//console.log("Questions:"+schemeObj[k].Question_No);	
					if(schemeObj[k].allotedMarks !== undefined){
							var gName ='group'+(k+1);
							//console.log("GroupName:"+gName);	
							var qmin=parseInt(schemeObj[k].MaxScore)-parseInt(schemeObj[k].MinScore);
							
							var garray=[];
						for(g=0;g<schemeObj.length;g++){
							if(schemeObj[g].Group==gName){
								if(schemeObj[g].allotedMarks==='' || schemeObj[g].allotedMarks==='NA'){
									garray.push(schemeObj[g].Question_No);
									
								}
							}
						}
						
						if(garray.length>qmin){
							//console.log("Questions:"+JSON.stringify(garray));
							for(p=0;p<garray.length;p++){
								unmarkquestion.push(garray[p]);
							}
						}
						//console.log("unmarkquestion:"+JSON.stringify(unmarkquestion));
						
					}else{
						if(schemeObj[k].Question_No!='condition'){
							unmarkquestion.push(schemeObj[k].Question_No);
						}
					}
				}else{
					if(schemeObj[k].allotedMarks !== undefined){
						if((schemeObj[k].allotedMarks==='' || schemeObj[k].allotedMarks==='NA') && schemeObj[k].Group==''){
							if(schemeObj[k].Question_No!='condition' ){
							   unmarkquestion.push(schemeObj[k].Question_No);
							}
						}
					}else{
						if(schemeObj[k].Group!='' && schemeObj[k].Question_No!='condition'){
							unmarkquestion.push(schemeObj[k].Question_No);
						}else{
							unmarkquestion.push(schemeObj[k].Question_No);
						}
			...
			if(unmarkquestion.length>0){
				alert('You have not marked anything on Question : '+unmarkquestion.join());
			}else{
		
		
        var unMarked = [];
        var url = base_url + 'sheet/saveImage';
...
function finalSave() {
    ...
    if ($('#confirmation').prop("checked") == true) {
        //var resp=getContent('sheet/savePdf?url='+sheet.url); 
        //alert('sheet/savePdf?url='+sheet.url);
        var resp = '';
        $.ajax({
            type: "POST",
            url: base_url + 'sheet/savePdf?url=' + sheet.url,
            async: false,
            data: {
                'json': schemeObj,
                'url': sheet.url
        ...
        if (resp.success) {
            ...
            if (isEval) {
                $.ajax({
                    type: "POST",
                    url: base_url + 'sheet/saveMarkingLog',
                    async: false,
                    data: {
                        'sheetObj': sheetMarking
                    }
                }).done(function(data) {
                    save = data;
                });
            ...
            if (!isEval) {
                ...
                $.ajax({
                    type: "POST",
                    url: base_url + 'sheet/save',
                    async: false,
                    data: {
                        'evaluation_json': schemeObj,
                        'sheet_file': sheet.sheet_file,
                        'evaluation_marks': totalScore,
                        'evalType': evalType
                    }
                }).done(function(data) {
                    save = data;
                ...
                if (save.success) {
                    getContent('sheet/deletePages?url=' + sheet.url, '', true);
...
function resetTimer() {
    ...
    var time = Date.now();
    timespent = (time - assignTime) / 1000;
    //alert(toHHMMSS(timespent));
}
...
function setScore() {
    ...
    if (linkContent == '') {
        var maxScore = parseFloat(schemeObj[selectedQue].MaxScore);
        var minScore = parseFloat(schemeObj[selectedQue].MinScore);
        var i = minScore;
...
function allotMark(score) {
    ...
    if (commentText != '') {
        ...
        if(resp.success){
            scorePage[selectedQue]=canvasId.substr(4);
            $('#queNum'+selectedQue).addClass('headMarked');
...
function rejectSheet() {
    ...
    if (error != '') {
        ...
        var resp = getContent('sheet/reject?sheet_file=' + sheet.sheet_file + '&reason=' + reason + '&other=' + otherText);
        alert(resp.message);
        if (resp.success) {
            top.location.href = '';
            window.location.href = "";
...
var allotedMarksSum = new Array();
var allotedMarksSumH = new Array();
...
function getSheet() {
    $("#evalMessage").html('Loading sheet, Please wait ...');
    sheet = getContent('sheet/get');

    if (sheet.success) {
        openFullscreen();
        $('#subjectName').html(sheet.subject_name);
		$('#subjectCode').html(sheet.subject_code);
        $('#sheetFile').html(sheet.sheet_id);
        $("#totalMarks").html(sheet.paper_total_marks);
        var details = getContent('sheet/details?url=' + sheet.url);
        var res = getContent('sheet/resolution?url=' + sheet.url);
        resetCanvas(res.size.width, res.size.height);

        var scheme = getContent('sheet/scheme?paper_code=' + sheet.paper_code);
        ...
        if (sheet.notAttempted) {
            for (i = 0; i < schemeObj.length; i++) {
                if (jQuery.inArray(i, sheet.notAttempted) >= 0) {
                    ...
                    cell2.innerHTML = schemeObj[i].MaxScore;
                    cell3.innerHTML = "<span id='queMark" + i + "'>-</span>";
        ...
        var tableSum = document.getElementById("summaryTableMarker");
        var rowCountSum = tableSum.rows.length;
        for (var i = rowCountSum - 1; i > 0; i--) {
            tableSum.deleteRow(i);
        }
        if (sheet.notAttempted) {
            for (i = 0; i < schemeObj.length; i++) {
                if (jQuery.inArray(i, sheet.notAttempted) >= 0) {
                    var row = tableSum.insertRow();
                    var cell1 = row.insertCell(0);
                    var cell2 = row.insertCell(1);
                    var cell3 = row.insertCell(2);
                    cell1.innerHTML = schemeObj[i].Question_No;
                    cell2.innerHTML = schemeObj[i].MaxScore;
                    cell3.innerHTML = "<span id='sumMark" + i + "'>-</span>";
                    schemeObj[i]['oldMarks'] = 'NA';
                }
                allotedMarksSum[i] = 'NA';
                if (schemeObj[i].Group != '') {
                    var gname = schemeObj[i].Group;
                    if (gname in groups) {
            ...
            cell1.innerHTML = "<b>Total</b>";
            cell2.innerHTML = sheet.paper_total_marks;
            cell3.innerHTML = "<b><span id='totalMark'>0</span></b>";
        }


        var tableSumH = document.getElementById("summaryTableHead");
        var rowCountSumH = tableSumH.rows.length;
        for (var i = rowCountSumH - 1; i > 0; i--) {
            tableSumH.deleteRow(i);
        }
        if (sheet.notAttempted) {
            for (i = 0; i < schemeObj.length; i++) {
                if (jQuery.inArray(i, sheet.notAttempted) >= 0) {
                    var row = tableSumH.insertRow();
                    var cell1 = row.insertCell(0);
                    var cell2 = row.insertCell(1);
                    var cell3 = row.insertCell(2);
                    cell1.innerHTML = schemeObj[i].Question_No;
                    cell2.innerHTML = schemeObj[i].MaxScore;
                    cell3.innerHTML = "<span id='sumMarkh" + i + "'>-</span>";
                    schemeObj[i]['oldMarks'] = 'NA';
                }
                allotedMarksSumH[i] = 'NA';
                if (schemeObj[i].Group != '') {
                    var gname = schemeObj[i].Group;
                    if (gname in groups) {
            ...
            cell1.innerHTML = "<b>Total</b>";
            cell2.innerHTML = sheet.paper_total_marks;
            cell3.innerHTML = "<b><span id='totalMarkh'>0</span></b>";
        }
        ...
        $('#docPageDiv1 img').attr('src', base_url + 'sheet/page/0?url=' + sheet.url);
        //first page actions draw
        var data = getContent('sheet/pageLog/1');

        for (k = 0; k < data.actions.length; k++) {
            oldData.push(data.actions[k]);
        }
        drawActions(data.actions);
        ...
        */
        for (i = 2; i <= totalPages['doc']; i++) {
            //var data=getContent('sheet/pageLog/'+i);
            //drawActions(data.actions);
            $('#docPageDiv' + i + ' img').attr('src', base_url + 'sheet/page/' + (i - 1) + '?url=' + sheet.url);
            getContent('sheet/pageLog/' + i, '', true, 'loadActions');
        }


        $("#evalMessage").html('Loading model answer, Please wait ...');
        getContent('sheet/details?url=' + sheet.paper_model_answer, '', true, 'loadModel');
...
function mSum() {
    $('#summaryHead').modal('show');
}

function mSumH() {
    $('#summaryMarker').modal('show');
}
...
function loadModel(ansDetails) {
    ...
    for (i = 1; i <= totalPages['ans']; i++) {
        $('#ansPageDiv' + i).show();
        $('#ansPageDiv' + i + ' img').attr('src', base_url + 'sheet/page/' + (i - 1) + '?url=' + sheet.paper_model_answer + '&&ans=ans');
...
function loadPaper(queDetails) {
    ...
    for (i = 1; i <= totalPages['que']; i++) {
        $('#quePageDiv' + i).show();
        $('#quePageDiv' + i + ' img').attr('src', base_url + 'sheet/page/' + (i - 1) + '?url=' + sheet.paper_model_question + '&&ans=ans');
...
function loadActions(data) {
    ...
    //console.log("ACtion:"+JSON.stringify(oldData));
    drawActions(data.actions);
}
...
function drawAction(action, scoreCalc = true) {
    ...
    if (drawTool == 'score') {
        ...
        if (action.user_role == 'Head_Marker') {
            if (scoreCalc) {
            ...
            $('#queNum' + action.eval_que_index).addClass('headMarked');
            $('#queMark' + action.eval_que_index).html(allotedMarks[action.eval_que_index]);
        } else if (action.user_role == 'Evaluator') {
            if (scoreCalc) {
                var score = parseFloat(detail.score);
                if (allotedMarksSum[action.eval_que_index] == 'NA') {
                    allotedMarksSum[action.eval_que_index] = 0;
                }
                allotedMarksSum[action.eval_que_index] += score;
                totalSumMarks += score;
                //calculateTotal();
            }
            $('#sumMark' + action.eval_que_index).html(allotedMarksSum[action.eval_que_index]);
            $("#totalMark").html(totalSumMarks);
        } else if (action.user_role == 'Head_Evaluator') {
            if (scoreCalc) {
                var score = parseFloat(detail.score);
                if (allotedMarksSumH[action.eval_que_index] == 'NA') {
                    allotedMarksSumH[action.eval_que_index] = 0;
                }
                allotedMarksSumH[action.eval_que_index] += score;
                totalSumMarksH += score;
                //calculateTotal();
            }
            $('#sumMarkh' + action.eval_que_index).html(allotedMarksSumH[action.eval_que_index]);
            $("#totalMarkh").html(totalSumMarksH);
        }
...
function undo() {
    ...
    if (sheetMarking.length > 0) {
        ...
        if (undoActionData.length > 0) {
            ...
            //console.log('Page:'+undoActionData[index].eval_page);
            var ppage = undoActionData[index].eval_page;
            if (data.length == 0) {
                $('#docpageNum' + undoActionData[index].eval_page).removeClass('marked');
            }
            var olddata = oldData.filter(function(v, i) {
                return v["eval_page"] == undoActionData[index].eval_page;
            });
            drawActions(olddata, false);
...
function rotate() {
    ...
    var data = getContent('sheet/rotate/' + (i - 1) + '?url=' + sheet.url);
