    ...
    if(display=='none'){
        ...
        var y=drag.top-offset.top;
            ...
            if(commentText.length>0){
				...
				eval_id++;
				drawCanvas(drawTool,pageId,x,y,width,height,commentText,'Head_Evaluator');
            }
...
function allotMark(score){
    ...
    if(commentText!=''){
		...
		scorePage[selectedQue]=canvasId.substr(4);
            ...
            drawCanvas(icon,canvasId,x,y,30,30,'Head_Evaluator');
            drawCanvas('comment',canvasId,x+30,y,width,height,commentText,'Head_Evaluator');
            drawCanvas('time',canvasId,x+30,y+22,width,height,currentDateTime(),'Head_Evaluator');
		//====================Added date on 29-08-2020===
    ...
    if(sheet.success){
       ...
       // $('#verifyCheckedSheet').modal('show');
		...
		for (var i = 0; i < data.actions.length; i++) {
			...
			var evalD=JSON.parse(actions[i].eval_details);
			var obj1={
					eval_id:eval_id,
					eval_page:actions[i].eval_page,
					eval_action:actions[i].eval_action,
					eval_details:JSON.stringify({x:evalD.x,y:evalD.y,width:evalD.width,height:evalD.height,commentText:evalD.commentText}),
					eval_que_index:actions[i].eval_que_index,
					sheet:sheet.sheet_file,
					eval_role:actions[i].eval_role,
					eval_time:actions[i].eval_time
				};
				sheetMarking.push(obj1);
				//addUndo(actions[i].eval_page,eval_id,actions[i].eval_que_index);
				eval_id++;
        }
        drawActions(data.actions);
...
function loadActions(data){
	...
	for (var i = 0; i < data.actions.length; i++) {
			...
			var evalD=JSON.parse(actions[i].eval_details);
			var evalDetails;
			if(actions[i].eval_action=='score'){
				evalDetails={x:evalD.x,y:evalD.y,width:evalD.width,height:evalD.height,score:evalD.score,commentText:evalD.commentText};
			}else{
				evalDetails={x:evalD.x,y:evalD.y,width:evalD.width,height:evalD.height,commentText:evalD.commentText};
			}
			var obj1={
					eval_id:eval_id,
					eval_page:actions[i].eval_page,
					eval_action:actions[i].eval_action,
					eval_details:JSON.stringify(evalDetails),
					eval_que_index:actions[i].eval_que_index,
					sheet:sheet.sheet_file,
					eval_role:actions[i].eval_role,
					eval_time:actions[i].eval_time
...
function drawAction(action,scoreCalc=true){
	//console.log("drawAction"+JSON.stringify(action));
            ...
            if(drawTool=='score'){
                ...
                drawCanvas(icon,pageId,x,y,30,30,'',action.eval_role);
                drawCanvas('comment',pageId,x+30,y,width,height,commentText,action.eval_role);
                drawCanvas('time',pageId,x+30,y+22,width,height,action.eval_time,action.eval_role);
            }else{
                drawCanvas(drawTool,pageId,x,y,width,height,commentText,action.eval_role);
            }
}
...
function undo(){
    ...
    if(sheetMarking.length>0){
		...
		//console.log("undoActionData:"+JSON.stringify(undoActionData[index]));
         var action=undoActionData[index];
		 redoStack.push(undoActionData[index]);
		 console.log("redoStack Redo:"+JSON.stringify(redoStack));
...
function redo(){
    if(redoStack.length>0){
		...
		//console.log("undoActionData Redo:"+JSON.stringify(undoActionData));
		if(undoActionData.length>0){
		var index=undoActionData.length-1;
