...
class Callbacks_Core
{
     ...
     * @param   array
     */
    function __construct($config, $language)
    {
        $this->config = $config;
    ...
    {
        $columns = $this->db->table_columns($table);
     ...
     * @return  array
     */
    function db_table_column($table, $column)
    {
        $columns = array_map('strtolower', $this->db->table_columns($table));
                        ...
                        if (!$comment) {
                            ...
                            if (substr(rtrim($query), -1) == ';') {
                                ...
                                if ($this->db_query($query) === false) {
                                    //$this->error = $this->db->error;
                                    $this->error = "database already exist or cannot run query";
                                    return false;
                                }
