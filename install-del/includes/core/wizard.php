...
class phpSetupWizard_Core
{
    ...
    function parse_step()
    {
        ...
        if (isset($_SESSION['redirect']) && ( !isset($this->step['redirect']) || !$this->step['redirect'] )) {
    ...
    function set_validate_error($validate, $field, $rule, $params)
    {
        ...
        if (isset($rule['error'])) {
            ...
            $validate->error = call_user_func_array('sprintf', array_merge(array($rule['error']), array($field['label']), $params));
        }
